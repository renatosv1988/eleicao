...
# excluir seções de cidades com passe livre no 1º turno
BD$passe_livre <- ifelse(BD$ANO_ELEICAO==2018, 0, BD$passe_livre)
BD$turnos_com_passe_livre <- ave(BD$passe_livre, BD$id_secao, FUN=sum)
BD <- BD[BD$turnos_com_passe_livre<=1,]

# apenas seções observadas em ambos os anos
BD <- BD[ave(BD$dummy_pt, BD$id_secao, FUN=sum)==4,]


# criar categorias do Triple-Diff
BD$tratado <- ave(BD$passe_livre, BD$id_secao, FUN=max)
BD$ano_2022 <- ifelse(BD$ANO_ELEICAO==2022,1,0)
