...
df_muni <- df_muni[QT_APTOS > 100000]

df_sections <- subset(df_sections, code_muni %in% df_muni$code_muni)
...
step1 <- glm(passe_livre_2 ~    gov_2t + name_region + biometria+ QT_APTOS_log + pib_log  + votos_jair_muni_validos_p ,
             family = binomial(link = 'logit'),
             data = df_muni)


summary(step1)
setDT(df_muni)[, ipw := (passe_livre_2 / fitted(step1)) + ((1 - passe_livre_2) / ( 1- fitted(step1)))]
