...
perfil <- fread('../../data/secoes/secoes_perfil_2010.csv')
votos <- fread('../../data/votes_2010/votos_T1_T2.csv')

eleicao_2010_raw <- fread('../../data/secoes/secoes_2010.csv')
eleicao_2010_raw[,id_secao := paste(CD_MUNICIPIO, NR_ZONA, NR_SECAO)]
...
detention_centers <- c('PRISIONAL|S?CIO EDUCATIVO|PENITENCI?RIO|INTERNA??O|UPR DE|UI/UIP|PRES?DIO|DETEN??O|PRIVA??O|PENAL|PENITENCIARIA|PENITENCI?RIA|SOCIOEDUCATIVO|CADEIA|FUNASE|CADE|CASE|VOTO EM TR?NSITO|PRESIDIO|PRISIONAL|CENTRO DE RECUPERA??O|SOCIO EDUCATIVO|IASES|CDPSM|FUNDA??O CASA|CDP|S?CIOEDUCATIVO|RESSOCIALIZA??O|PENINTENCI?RIO|RESSOCIALIZA??O')
...
eleicao_2010 <- eleicao_2010_raw[ id_secao %nin% id_secao_detention2022]
nrow(eleicao_2010)
#> 855408
...
eleicao_2010 <- merge(eleicao_2010, espacial, by="id_secao", all.x = T)

summary(eleicao_2010$num_1000)
#> Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#>  1.0    16.0    35.0    46.4    67.0   313.0   14440


eleicao_2010[is.na(num_1000)] |> nrow() / nrow(eleicao_2010)
# 0.1790514
...
eleicao_2010 <- merge(eleicao_2010, munic[,c("CD_MUNICIPIO", "dummy_pt", "code_muni")],
                      by="CD_MUNICIPIO", all.x = T)

table(eleicao_2010$dummy_pt, useNA = 'always')
#>       0      1   <NA> 
#>  244096 611312      0 
...
eleicao_2010 <- merge(eleicao_2010, perfil[,c("id_secao","mulheres","educacao_1",
                                              "idade_16_17","idade_18_24","idade_60M")],
                      by="id_secao", all.x = T)

summary(eleicao_2010$educacao_1)
#>    Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#>  0.0000  0.4091  0.5801  0.5590  0.7220  1.0000     432 

# proportion of missing
eleicao_2010[ is.na(educacao_1) , .N] / nrow(eleicao_2010)
#> 0.0005050222
...
eleicao_2010 <- rbind(t1, t2)

# replace NAs with 0
eleicao_2010[is.na(passe_livre), passe_livre := 0]


table(eleicao_2010$passe_livre, useNA = 'always')
#>       0      1   <NA> 
#>  596685 258723      0 

table(eleicao_2010$passe_livre_always, useNA = 'always')
#>     1   <NA> 
#>  3516 851892 


# MERGE PIB_PC 2019 ------------------------------------------------------------
eleicao_2010 <- merge(eleicao_2010, pib, by="code_muni")

summary(eleicao_2010$PIB_PC)
...
eleicao_2010[, code_region := substring(code_muni, 1, 1) |> as.numeric() ]
eleicao_2010 <- left_join(eleicao_2010, regions, by=c('code_region'))
table(eleicao_2010$name_region)

# adicionar votacao por candidato ----------------------------------------------

# merge data
eleicao_2010 <- merge(eleicao_2010, 
                          votos, 
                          by=c("ANO_ELEICAO", "SG_UF", "CD_MUNICIPIO", "NR_TURNO","id_secao"), 
                          all.x = T)

summary(eleicao_2010$votos_lula)
#>    Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's  
#>     0.0    75.0   109.0   114.2   149.0   429.0       6

summary(eleicao_2010$votos_total)
#>   Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#>    1.0   228.0   279.0   265.8   313.0   607.0       6 

summary(eleicao_2010$comparecimento_2010)
#>   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#> 0.0000  0.7575  0.8042  0.7981  0.8458  1.0000 


# adicionar variacao de comparecimento por municipio em 2010 -------------------

# separar por turno
T1 <- eleicao_2010[NR_TURNO==1,]
T2 <- eleicao_2010[NR_TURNO==2,]
...
MM[, variacao_comparecimento_2010 := comparecimento_t2 - comparecimento_t1]

# merge na base principal
eleicao_2010 <- merge(eleicao_2010,
                      MM[,c("id_secao","variacao_comparecimento_2010")],
                      by="id_secao", all.x = T)

summary(eleicao_2010$variacao_comparecimento_2010)
...
my_var <- c("id_secao",  "CD_MUNICIPIO","NR_ZONA", "NR_SECAO",
            ...
            "mulheres","educacao_1",
            "idade_16_17","idade_18_24","idade_60M",
            "comparecimento_2010","abstencao_2010",
            
            "variacao_comparecimento_2010",  
...
fwrite(eleicao_2010, "../../data/base_DiD2010_secoes.csv")
saveRDS(eleicao_2010, "../../data/base_DiD2010_secoes.rds",compress = T)
