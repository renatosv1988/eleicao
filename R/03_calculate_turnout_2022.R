...
# read data
files_eleitorado <- list.files(path = './data_raw/eleitorado', pattern = '2022.csv', full.names = T)
...
# read data
files_zonas <- list.files(path = './data_raw/zonas', pattern = '2022.csv', full.names = T)
...
df_zonas <- zonas[, .(QT_ELEITOR_SECAO=sum(QT_ELEITOR_SECAO),
                      QT_ELEITOR_ELEICAO_FEDERAL=sum(QT_ELEITOR_ELEICAO_FEDERAL)), 
                  by = .(SG_UF, CD_MUNICIPIO, NM_MUNICIPIO, 
                         NR_ZONA,  # NR_SECAO,
                         NR_TURNO
...
df_zonas_aggreg_mun <- zonas[, .(QT_ELEITOR_SECAO=sum(QT_ELEITOR_SECAO),
                                 QT_ELEITOR_ELEICAO_FEDERAL=sum(QT_ELEITOR_ELEICAO_FEDERAL)), 
...
files_votacao <- list.files(path = './data_raw/votacao', 
                            pattern = 'votacao_candidato_munzona_2022_BR.csv', 
                            full.names = T)
...
df_votacao_municipio <- votacao_pres[, .(QT_VOTOS_NOMINAIS = sum(QT_VOTOS_NOMINAIS),
                               QT_VOTOS_NOMINAIS_VALIDOS = sum(QT_VOTOS_NOMINAIS_VALIDOS)), 
                      by = .(SG_UF, CD_MUNICIPIO, NM_MUNICIPIO, 
                             # NR_ZONA,
                             NR_TURNO, DS_CARGO
                             #, SG_PARTIDO,
...
compar_muni[, comparecimento := QT_VOTOS_NOMINAIS / QT_ELEITOR_ELEICAO_FEDERAL]
...
compar_zona[, comparecimento := QT_VOTOS_NOMINAIS / QT_ELEITOR_ELEICAO_FEDERAL]
...
fwrite(compar_muni_edu, './data/comparecimento_munis_2022.csv')
fwrite(compar_zona_edu, './data/comparecimento_zonas_2022.csv')
